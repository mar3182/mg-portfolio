...
const Blog = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Header Section */}
      <section className="blog-section" style={{ paddingBottom: '4rem' }}>
      ...
      <section className="px-6" style={{ paddingBottom: '4rem' }}>
        <div className="blog-inner">
          <RevealWrapper>
            <div className="bg-slate-50 rounded-lg border border-slate-200" style={{ padding: '2.5rem' }}>
              {/* Search Bar */}
          ...
          <div className="blog-inner">
            <RevealWrapper>
              ...
              <Link to={`/blog/${featuredPost.slug}`} className="block group focus-ring-link">
                <article className="bg-slate-50 rounded-lg border border-slate-200 transition-all duration-300 hover:shadow-lg hover:border-slate-300 w-full" style={{ padding: '3rem' }}>
                  <div className="flex flex-wrap gap-2" style={{ marginBottom: '2rem' }}>
                    {featuredPost.categories.map((category) => (
                      <span key={category} className="px-3 py-1 bg-yellow-100 text-yellow-700 text-xs font-medium rounded-full">
                        {category}
                      </span>
                    ))}
                  </div>
                  
                  <h3 className="text-2xl md:text-3xl font-bold text-slate-900 group-hover:text-yellow-600 transition-colors" style={{ marginBottom: '2rem' }}>
                    {featuredPost.title}
                  </h3>
                  
                  <p className="text-lg text-slate-600 leading-relaxed" style={{ marginBottom: '2.5rem' }}>
      ...
      <section className="pb-24 px-6">
        <div className="blog-inner">
          ...
          {filteredPosts.length === 0 ? (
            ...
            <StaggerWrapper className="mt-10 md:mt-14">
              <div
                ref={gridRef}
                className="blog-grid"
                style={{
                ...
                {(searchActive || !featuredPost ? filteredPosts : regularPosts).map((post) => (
                  <StaggerItem key={post.id}>
                    <Link to={`/blog/${post.slug}`} className="block h-full group focus-ring-link">
                      <article 
                        className="h-full bg-white rounded-xl border border-slate-200/80 transition-all duration-300 hover:shadow-xl hover:border-slate-300 flex flex-col"
                        style={{
                          padding: '3rem',
                          marginBottom: '2rem',
                        }}
                      >
                        <div className="flex flex-wrap gap-2" style={{ marginBottom: '2rem' }}>
                          {post.categories.slice(0, 2).map((category) => (
                            <span key={category} className="px-2 py-1 bg-yellow-100 text-yellow-700 text-xs font-medium rounded-full">
                              {category}
                            </span>
                          ))}
                        </div>
                        
                        <h3 className="text-[20px] md:text-xl font-bold text-slate-900 group-hover:text-yellow-600 transition-colors leading-snug" style={{ marginBottom: '2.5rem' }}>
                          {post.title}
                        </h3>
                        
                        <p className="text-slate-600 leading-relaxed flex-grow" style={{ marginBottom: '3rem' }}>
                          {post.excerpt}
                        </p>
                        
                        <div className="flex items-center justify-between text-sm text-slate-500 border-t border-slate-200" style={{ paddingTop: '2rem', marginTop: '1rem' }}>
