      ...
      .map((paragraph, index) => {
        ...
        const processInlineCode = (text) => {
          return text.split('`').map((part, i) => 
            i % 2 === 1 ? 
              <code key={i} className="bg-yellow-400/80 dark:bg-yellow-500/80 px-2 py-1 text-sm text-black dark:text-black">{part}</code> : 
              part
          )
        }
          ...
          <RevealWrapper>
            <div className="flex flex-wrap gap-2" style={{ marginBottom: '2rem' }}>
              {post.categories.map((category) => (
                <span key={category} className="px-3 py-1 bg-yellow-400/80 dark:bg-yellow-500/80 text-black dark:text-black text-sm font-medium">
                  {category}
                </span>
              ))}
            </div>
          ...
          <RevealWrapper>
            <div className="bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-200/50 dark:border-slate-700/50" style={{ padding: '2rem' }}>
              ...
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 bg-yellow-400/80 dark:bg-yellow-500/80 text-black dark:text-black text-sm">
                    #{tag}
                  </span>
                ))}
              </div>
                    ...
                    >
                      <div className="flex flex-wrap gap-2 mb-5">
                        {relatedPost.categories.slice(0, 2).map((category) => (
                          <span key={category} className="px-2 py-1 bg-yellow-400/80 dark:bg-yellow-500/80 text-black dark:text-black text-xs font-medium">
                            {category}
                          </span>
                        ))}
                      </div>
