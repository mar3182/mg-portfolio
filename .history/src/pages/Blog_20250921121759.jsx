                ...
                {(searchActive || !featuredPost ? filteredPosts : regularPosts).map((post) => (
                  <StaggerItem key={post.id}>
                    <Link to={`/blog/${post.slug}`} className="block h-full group focus-ring-link">
                      <article className="h-full bg-white rounded-xl p-9 md:p-10 border border-slate-200/80 transition-all duration-300 hover:shadow-xl hover:border-slate-300 flex flex-col">
                        ...
                        <h3 className="text-[20px] md:text-xl font-bold text-slate-900 mb-7 group-hover:text-blue-600 transition-colors leading-snug">
                          {post.title}
                        </h3>
                        ...
                        <div className="flex items-center justify-between text-sm text-slate-500 pt-6 mt-1 border-t border-slate-200">
