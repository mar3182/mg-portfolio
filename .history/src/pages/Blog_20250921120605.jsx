...
const Blog = () => {
  ...
  const [gridDebug, setGridDebug] = useState('')

  useEffect(() => {
    const el = gridRef.current
    if (!el) return
    // Delay a tick to ensure styles are applied
    const id = setTimeout(() => {
      const cs = window.getComputedStyle(el)
      setGridDebug(`display=${cs.display} | cols=${cs.gridTemplateColumns} | gap=${cs.gap} | col-gap=${cs.columnGap} | row-gap=${cs.rowGap}`)
    }, 0)
      ...
      <section className="pb-20 px-6">
        <div className="blog-inner">
          ...
          {filteredPosts.length === 0 ? (
            ...
            <StaggerWrapper className="mt-8 md:mt-12">
              <div className="text-xs text-slate-500 mb-2" aria-live="polite">Grid debug: {gridDebug}</div>
              <div
                ref={gridRef}
                className="blog-grid"
                style={{
                  display: 'grid',
                  gridTemplateColumns: '1fr', // DEBUG: force single column
                  columnGap: '10rem',
                  rowGap: '6.5rem',
                  outline: '4px solid rgba(255, 0, 102, 0.5)',
                ...
                {(searchActive || !featuredPost ? filteredPosts : regularPosts).map((post) => (
                  <StaggerItem key={post.id}>
                    <Link to={`/blog/${post.slug}`} className="block h-full group focus-ring-link">
                      <article className="h-full bg-white rounded-xl p-8 border border-slate-200/80 transition-all duration-300 hover:shadow-xl hover:border-slate-300 flex flex-col">
                        ...
                        <h3 className="text-[20px] md:text-xl font-bold text-slate-900 mb-6 group-hover:text-blue-600 transition-colors leading-snug">
                          {post.title}
                        </h3>
                        ...
                        <div className="flex items-center justify-between text-sm text-slate-500 pt-5 mt-1 border-t border-slate-200">
