...
export default function Home() {
  ...
  // Load showcase rotator when right pane scrolled into view (root margin generous)
  useEffect(() => {
    const el = document.querySelector('.showcase-stack')
    if (!el) return
    const obs = new IntersectionObserver((entries) => {
      if (entries.some(e => e.isIntersecting)) {
        import('../components/ShowcaseRotator').then(m => setShowcaseRotatorComp(()=>m.default)).catch(()=>{})
        obs.disconnect()
      }
    }, { rootMargin: '600px' })
    obs.observe(el)
