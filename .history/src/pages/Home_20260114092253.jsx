...
export default function Home() {
  ...
  const [ShowcaseCarousel3DComp, setShowcaseCarousel3DComp] = useState(null)
  ...
  // Load 3D carousel when right pane scrolled into view (root margin generous)
  useEffect(() => {
    const el = document.querySelector('.showcase-stack')
    if (!el) return
    const obs = new IntersectionObserver((entries) => {
      if (entries.some(e => e.isIntersecting)) {
        import('../components/ShowcaseCarousel3D').then(m => setShowcaseCarousel3DComp(()=>m.default)).catch(()=>{})
        obs.disconnect()
      }
    }, { rootMargin: '600px' })
    obs.observe(el)
  ...
  <section id="home" className="split-screen sticky-left-layout" aria-label="Home" data-stack-zone>
      <div className="split-left hero-left sticky-pane">
        ...
        <p className="hero-sub">
          Nous façonnons des identités et des expériences numériques avec une obsession pour les détails et la performance.
        </p>
        <a
          href="/projects"
          className="hero-cta"
  ...
  {ShowcaseCarousel3DComp && <ShowcaseCarousel3DComp onIndexChange={() => { /* ambient color side-effect handled inside component */ }} />}
        </RevealWrapper>
      </div>
  </section>
