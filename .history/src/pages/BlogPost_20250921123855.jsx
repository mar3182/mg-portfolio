...
const BlogPost = () => {
    ...
    return content
      ...
      .map((paragraph, index) => {
        ...
        if (paragraph.startsWith('# ')) {
          return <h1 key={index} className="text-4xl font-bold text-slate-800 dark:text-slate-200 leading-tight" style={{ marginBottom: '2rem', marginTop: index > 0 ? '3rem' : '0' }}>{paragraph.slice(2)}</h1>
        }
        if (paragraph.startsWith('## ')) {
          return <h2 key={index} className="text-3xl font-bold text-slate-800 dark:text-slate-200 leading-tight" style={{ marginBottom: '1.5rem', marginTop: index > 0 ? '2.5rem' : '0' }}>{paragraph.slice(3)}</h2>
        }
        if (paragraph.startsWith('### ')) {
          return <h3 key={index} className="text-2xl font-semibold text-slate-800 dark:text-slate-200 leading-tight" style={{ marginBottom: '1.25rem', marginTop: index > 0 ? '2rem' : '0' }}>{paragraph.slice(4)}</h3>
        }
        ...
        if (paragraph.startsWith('```')) {
          ...
          return (
            <div key={index} style={{ marginBottom: '2.5rem', marginTop: '1.5rem' }}>
              <pre className="bg-slate-900 text-slate-100 rounded-xl overflow-x-auto" style={{ padding: '2rem', fontSize: '0.9rem', lineHeight: '1.6' }}>
        ...
        if (paragraph.includes('\n- ')) {
          ...
          return (
            <ul key={index} className="list-disc list-inside text-slate-600 dark:text-slate-300" style={{ marginBottom: '2rem', paddingLeft: '1rem' }}>
              {items.map((item, itemIndex) => (
                <li key={itemIndex} className="leading-relaxed" style={{ marginBottom: '0.75rem' }}>{item.replace(/^- /, '')}</li>
              ))}
            </ul>
          )
        }
        ...
        if (/^\d+\./.test(paragraph)) {
          ...
          return (
            <ol key={index} className="list-decimal list-inside text-slate-600 dark:text-slate-300" style={{ marginBottom: '2rem', paddingLeft: '1rem' }}>
              {items.map((item, itemIndex) => (
                <li key={itemIndex} className="leading-relaxed" style={{ marginBottom: '0.75rem' }}>{item.replace(/^\d+\.\s*/, '')}</li>
              ))}
            </ol>
          )
        }
        ...
        const processInlineCode = (text) => {
          return text.split('`').map((part, i) => 
            i % 2 === 1 ? 
              <code key={i} className="bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-sm text-yellow-600 dark:text-yellow-400">{part}</code> : 
              part
          )
        }
        ...
        if (paragraph.trim()) {
          return (
            <p key={index} className="text-slate-600 dark:text-slate-300 leading-relaxed" style={{ marginBottom: '1.75rem', fontSize: '1.1rem', lineHeight: '1.7' }}>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
      {/* Back Navigation */}
      <div className="px-6" style={{ paddingTop: '6rem', paddingBottom: '3rem' }}>
        <div className="blog-inner">
          <RevealWrapper>
            <Link 
              to="/blog" 
              className="inline-flex items-center gap-2 text-yellow-600 dark:text-yellow-400 hover:text-yellow-700 dark:hover:text-yellow-300 transition-colors group focus-ring-link"
      ...
      {/* Article Header */}
      <header className="px-6" style={{ paddingBottom: '4rem' }}>
        <div className="blog-inner">
          <RevealWrapper>
            <div className="flex flex-wrap gap-2" style={{ marginBottom: '2rem' }}>
              {post.categories.map((category) => (
                <span key={category} className="px-3 py-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 text-sm font-medium rounded-full">
                  {category}
                </span>
              ))}
            </div>
            ...
            <Motion.h1 
              className="text-4xl md:text-5xl lg:text-6xl font-bold text-slate-800 dark:text-slate-200 leading-tight"
            ...
            <p className="text-xl text-slate-600 dark:text-slate-300 leading-relaxed" style={{ marginBottom: '2.5rem' }}>
              {post.excerpt}
            </p>
          ...
          <section className="px-6" style={{ paddingBottom: '3rem' }}>
            <div className="blog-inner">
              <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm rounded-2xl border border-slate-200/50 dark:border-slate-700/50 flex flex-col md:flex-row md:items-center md:justify-between gap-4" style={{ padding: '2rem' }}>
      ...
      {/* Article Content */}
      <main className="px-6" style={{ paddingBottom: '4rem' }}>
        <div className="blog-inner">
          <RevealWrapper>
            <article className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm rounded-2xl border border-slate-200/50 dark:border-slate-700/50 prose prose-slate dark:prose-invert max-w-3xl mx-auto prose-lg prose-headings:mb-6 prose-headings:mt-8 prose-p:mb-6 prose-p:leading-relaxed prose-li:mb-2 prose-ul:mb-6 prose-ol:mb-6 prose-pre:mb-8 prose-blockquote:mb-8" style={{ padding: '4rem 3rem' }}>
      ...
      <section className="px-6" style={{ paddingBottom: '4rem' }}>
        <div className="blog-inner">
          <RevealWrapper>
            <div className="bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-200/50 dark:border-slate-700/50" style={{ padding: '2rem' }}>
              <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200" style={{ marginBottom: '1.5rem' }}>Tags</h3>
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 bg-yellow-100 dark:bg-yellow-700 text-yellow-700 dark:text-yellow-300 text-sm rounded-full">
                    #{tag}
                  </span>
                ))}
              </div>
      ...
      {recentPosts.length > 0 && (
        <section className="px-6" style={{ paddingBottom: '5rem' }}>
          <div className="blog-inner">
            <RevealWrapper>
              <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200" style={{ marginBottom: '3rem' }}>More Articles</h2>
                    ...
                    >
                      <div className="flex flex-wrap gap-2 mb-5">
                        {relatedPost.categories.slice(0, 2).map((category) => (
                          <span key={category} className="px-2 py-1 bg-yellow-100 dark:bg-yellow-700 text-yellow-700 dark:text-yellow-300 text-xs font-medium rounded-full">
                            {category}
                          </span>
                        ))}
                      </div>
                      
                      <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-4 group-hover:text-yellow-600 dark:group-hover:text-yellow-400 transition-colors leading-tight">
                        {relatedPost.title}
                      </h3>
