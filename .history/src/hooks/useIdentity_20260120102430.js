...
/**
 * Get a smoothed identity value for BACKGROUND/ATMOSPHERE use
 * Uses very slow, heavy spring - backgrounds should drift, not snap
 * 
 * AWARD-LEVEL RULE: Background motion slower than portrait, slower than cursor
 * 2-4s effective response time
 */
export function useSmoothedIdentity() {
  // Very slow, heavy spring for atmospheric drift
  // This is intentionally sluggish - creates cinematic confidence
  return useSpring(identityMotionValue, {
    stiffness: 15,   // Very soft
    damping: 20,     // Smooth deceleration
    mass: 2,         // Heavy = slow to react
  })
}
