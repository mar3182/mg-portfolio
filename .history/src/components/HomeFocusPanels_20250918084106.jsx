...
export default function HomeFocusPanels({ onFocusChange }) {
  ...
  const rightZoneActiveRef = useRef(false)
  const rafRef = useRef(null)

  // Global mouse position listener to trigger stacking effect when cursor is in right half of viewport
  useEffect(() => {
    const hasFinePointer = window.matchMedia('(pointer: fine)').matches
    if (!hasFinePointer) return // avoid on touch devices

    const handleMove = (e) => {
      if (rafRef.current) cancelAnimationFrame(rafRef.current)
      rafRef.current = requestAnimationFrame(() => {
        const inRightHalf = e.clientX > window.innerWidth * 0.5
        if (inRightHalf !== rightZoneActiveRef.current) {
          rightZoneActiveRef.current = inRightHalf
          if (inRightHalf) {
            document.body.classList.add('showcase-card-hovered')
          } else {
            // Only remove if no card explicitly hovered (other components might add class)
          ...
          {focusAreas.map((area, i) => {
            ...
            return (
              <Motion.div
                ...
                onMouseEnter={() => {
                  setHoveredPanel(i)
                  // ensure class present immediately for fast entry at boundary
                  document.body.classList.add('showcase-card-hovered')
                }}
                onMouseLeave={() => {
                  setHoveredPanel(null)
                  // Only remove if cursor also left right half zone
                  if (!rightZoneActiveRef.current) {
                    document.body.classList.remove('showcase-card-hovered')
                  }
                }}
                onClick={() => {
