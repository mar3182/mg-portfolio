...
// Skills and project data for each focus area
const focusAreaContent = {
  strategy: {
    skills: ['Market Research', 'Competitor Analysis', 'Brand Positioning', 'Growth Strategy'],
    projects: projects.filter(p => p.category.includes('Portfolio') || p.category.includes('SaaS')),
    metrics: { completion: 95, satisfaction: 4.8, projects: 12 }
  },
  design: {
    skills: ['UI/UX Design', 'Brand Identity', 'Visual Systems', 'Prototyping'],
    projects: projects.filter(p => p.category.includes('Design') || p.category.includes('Portfolio')),
    metrics: { completion: 92, satisfaction: 4.9, projects: 18 }
  },
  commerce: {
    skills: ['E-commerce Development', 'Shopify', 'Payment Integration', 'Performance Optimization'],
    projects: projects.filter(p => p.category.includes('Development') || p.category.includes('WordPress')),
    metrics: { completion: 98, satisfaction: 4.7, projects: 25 }
  },
  experience: {
    skills: ['Interactive Design', 'Animation', 'WebGL', 'Performance'],
    projects: projects.filter(p => p.category.includes('Interactive') || p.category.includes('Web')),
    metrics: { completion: 88, satisfaction: 4.6, projects: 15 }
  }
}
