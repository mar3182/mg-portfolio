...
export function useHash(hashKey){
...
export default function ExpertiseDetailModal(){
  ...
  useEffect(()=>{ if(data){ document.documentElement.style.setProperty('--accent-color', data.color) } },[data])

  return (
    <AnimatePresence>
      {data && (
        <Motion.div className="project-preview-modal" role="dialog" aria-modal="true" aria-label={data.title + ' details'} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
