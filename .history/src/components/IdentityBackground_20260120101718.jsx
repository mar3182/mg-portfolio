...
function IdentityBackground() {
  ...
  const backgroundColor = useTransform(
    smoothIdentity,
    [0, 0.35, 0.5, 0.65, 1],
    [DESIGN_BG, DESIGN_BG_DARK, NEUTRAL_BG, TECH_BG_MID, TECH_BG]
  )
  
  // Grain opacity - more visible on light, subtle on dark
  const grainOpacity = useTransform(smoothIdentity, [0, 0.5, 1], [0.08, 0.04, 0.015])
  
  // Vignette intensity - stronger on edges for depth
  const vignetteOpacity = useTransform(smoothIdentity, [0, 0.5, 1], [0.3, 0.5, 0.4])
  
  // Warm glow for design side
  const warmGlowOpacity = useTransform(smoothIdentity, [0, 0.4, 0.6], [0.4, 0.1, 0])
  
  // Cool glows for tech side - multiple layers for depth
  const blueGlowOpacity = useTransform(smoothIdentity, [0.5, 0.7, 1], [0, 0.15, 0.25])
  const purpleGlowOpacity = useTransform(smoothIdentity, [0.5, 0.75, 1], [0, 0.1, 0.2])
  const cyanGlowOpacity = useTransform(smoothIdentity, [0.6, 0.85, 1], [0, 0.08, 0.15])
  ...
  return (
    <div className="identity-background" aria-hidden="true">
      ...
      {/* Tech side - Deep navy blue glow (top right) */}
      <Motion.div 
        className="identity-bg__gradient identity-bg__gradient--tech-blue"
        style={{
          opacity: blueGlowOpacity,
          background: `radial-gradient(ellipse 80% 60% at 85% 20%, ${TECH_ACCENT_BLUE} 0%, transparent 70%)`,
        }}
      />
      
      {/* Tech side - Deep purple glow (bottom left) */}
      <Motion.div 
        className="identity-bg__gradient identity-bg__gradient--tech-purple"
        style={{
          opacity: purpleGlowOpacity,
          background: `radial-gradient(ellipse 70% 80% at 15% 80%, ${TECH_ACCENT_PURPLE} 0%, transparent 65%)`,
        }}
      />
      
      {/* Tech side - Cyan/teal accent (center right) */}
      <Motion.div 
        className="identity-bg__gradient identity-bg__gradient--tech-cyan"
        style={{
          opacity: cyanGlowOpacity,
          background: `radial-gradient(ellipse 50% 50% at 90% 50%, ${TECH_ACCENT_CYAN} 0%, transparent 60%)`,
        }}
      />
      ...
      <Motion.div 
        ...
        style={{
          opacity: useTransform(smoothIdentity, [0.6, 1], [0, 0.02]),
        }}
      />
    </div>
  )
