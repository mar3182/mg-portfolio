...
import { useIdentityActions, useIdentityMotion, useDelayedIdentity, useVerticalScrollUnlock } from '../hooks/useIdentity'
...
export default function SplitPortraitHero({ 
  ...
  const [showOrientationCue, setShowOrientationCue] = useState(true)
  
  // ═══ IDENTITY SYSTEM INTEGRATION ═══
  // Mouse movement in this hero drives the global identity value
  const { setIdentity, animateToIdentity } = useIdentityActions()
  const motionConfig = useIdentityMotion()
  
  // Delayed identity for portrait crossfades (80-120ms lag per spec)
  const delayedIdentity = useDelayedIdentity(100)
  ...
  useEffect(() => {
    if (isReady) {
      ...
      // Fade out orientation labels after 3 seconds (per T-Shaped spec)
  ...
  // Smooth spring animation - use identity-aware spring config from spec
  // Stiffness: 40-60, Damping: 25-30, Mass: ~1
  const springConfig = { 
    stiffness: motionConfig.stiffness, 
    damping: motionConfig.damping, 
    mass: motionConfig.mass 
  }
  const smoothX = useSpring(mouseX, springConfig)
  const smoothY = useSpring(mouseY, springConfig)

  // === PORTRAIT OPACITY CROSSFADES ===
  // Uses DELAYED identity for cinematic "world reacts, she responds" feel
  // Design: Full at 0, fades out by 0.35
  const designOpacity = useTransform(delayedIdentity, [0, 0.25, 0.4], [1, 0.8, 0])
  
  // Split: Appears 0.3-0.7 range (center)
  const splitOpacity = useTransform(delayedIdentity, [0.25, 0.4, 0.6, 0.75], [0, 1, 1, 0])
  
  // Tech: Full at 1, fades out by 0.65
  const techOpacity = useTransform(delayedIdentity, [0.6, 0.75, 1], [0, 0.8, 1])
  ...
  return (
    ...
    >
      ...
      {/* Exploration hint - fades after interaction OR after orientation cue timer */}
      <Motion.div 
        className="explore-hint"
        animate={{ opacity: (hasInteracted || !showOrientationCue) ? 0 : 0.6 }}
        transition={{ duration: 0.8 }}
      >
