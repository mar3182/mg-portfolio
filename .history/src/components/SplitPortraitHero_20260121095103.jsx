...
export default function SplitPortraitHero({ 
  ...
  // === IDENTITY TITLE OPACITY & PARALLAX ===
  // Design title: fades in as user moves left, drifts further left (parallax)
  const designTitleOpacity = useTransform(smoothX, [0.5, 0.2, 0], [0, 0.5, 0.75])
  const designTitleX = useTransform(smoothX, [0.5, 0], ['20px', '-30px'])
  const designTitleScale = useTransform(smoothX, [0.5, 0], [0.9, 1])
  
  // Tech title: fades in as user moves right, drifts further right (parallax)
  const techTitleOpacity = useTransform(smoothX, [0.5, 0.8, 1], [0, 0.5, 0.75])
  const techTitleX = useTransform(smoothX, [0.5, 1], ['-20px', '30px'])
  const techTitleScale = useTransform(smoothX, [0.5, 1], [0.9, 1])
  ...
  return (
    ...
    >
      ...
      <Motion.div 
        ...
        style={{ x: scenePanX }}
      >
        {/* === IDENTITY TITLES — Award-Level Typography with Parallax === */}
        <Motion.div 
          className="identity-title identity-title--design"
          style={{ 
            opacity: designTitleOpacity,
            x: designTitleX,
            scale: designTitleScale,
          }}
        >
        ...
        <Motion.div 
          ...
          style={{ 
            opacity: techTitleOpacity,
            x: techTitleX,
            scale: techTitleScale,
          }}
        >
          <span className="title-text">Tech</span>
        </Motion.div>
        
        {/* === MINIMAL POSITION INDICATOR === */}
        {/* Single subtle dot — no track, no labels, just presence */}
        <Motion.div 
          className="position-dot-container"
          initial={{ opacity: 0 }}
          animate={{ opacity: 0.4 }}
