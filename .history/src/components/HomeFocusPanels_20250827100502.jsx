  ...
  useEffect(()=>{
    const handler = (e)=>{
      if(['1','2','3','4'].includes(e.key)){
        ...
        if(idx < focusAreas.length){ setActive(idx); onFocusChange?.(focusAreas[idx].expKey); goToArea(idx) }
      }
    }
      ...
      {focusAreas.map((area,i)=>{
        ...
        return (
          <Motion.button
            ...
            animate={{ opacity:1, y:0, flex:isActive?1:undefined }}
            whileHover={{ scale: isActive?1.02:1.015 }}
            transition={{ delay: 0.15 + i*0.05 }}
            onClick={()=>{ setActive(i); goToArea(i); onFocusChange?.(focusAreas[i].expKey) }}
          >
