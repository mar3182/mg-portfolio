...
class Vine {
  ...
  grow(dt, waveTime) {
    ...
    const currentLength = this.maxLength * Math.min(1, this.growthProgress)
...
function PortraitEcosystem() {
  ...
  const spawnFloraDetails = useCallback((canvas) => {
    const entities = entitiesRef.current
  ...
  const spawnCircuitDetails = useCallback((canvas) => {
    const entities = entitiesRef.current
