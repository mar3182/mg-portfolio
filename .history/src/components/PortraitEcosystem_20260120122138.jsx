...
function PortraitEcosystem() {
  ...
  const animate = useCallback((time) => {
    ...
    entities.flowers.forEach(flower => {
      ...
      flower.draw(ctx, Math.min(0.7, floraBaseOpacity + boost), entities.waveTime) // Cap at 70% opacity
    })
    
    entities.leaves.forEach(leaf => {
      const mouseX = mouse ? mouse.x * canvas.width : -1000
      const mouseY = mouse ? mouse.y * canvas.height : -1000
      leaf.update(dt, mouseX, mouseY)
