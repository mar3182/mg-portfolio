...
const CONFIG = {
  ...
  // Flora settings — FRIDA KAHLO JUNGLE STYLE
  // Rich, lush, layered tropical botanical garden
  flora: {
    maxVines: 25,
    maxFlowers: 35,
    maxLeaves: 120, // Many more leaves for jungle density
    maxTropicalLeaves: 40, // Large monstera/palm leaves
    maxFerns: 30, // Delicate fern fronds
    growthSpeed: 0.0008,
    waveSpeed: 0.001,
    waveAmount: 15,
    // Frida Kahlo palette — deep jungle greens, warm earth tones, bold accents
    colors: {
      vine: ['#2d5a3d', '#3a6b4a', '#4a7a5a', '#1a4a2d'],
      flower: [
        '#e85a5a', '#f06a4a', '#ff8c5a', // Warm reds/oranges
        '#e84a8a', '#d43a7a', '#ff6b9d', // Magentas/pinks
        '#ffc93c', '#ffab2e', '#f4d03f', // Golden yellows
        '#ff7b54', '#ff5733', // Bold oranges
      ],
      leaf: [
        '#1a4a2d', '#2d5a3d', '#3a6b4a', // Deep jungle greens
        '#4a7a5a', '#5a8a6a', '#3d6b4d', // Mid greens
        '#2a5a4a', '#3a7a5a', '#4d8a6a', // Teal-greens
      ],
      tropicalLeaf: [
        '#1a3a2a', '#2a4a3a', '#1d4d35', // Very dark (back layer)
        '#2d5a4a', '#3a6a5a', '#2a5a3d', // Dark (mid layer)
        '#4a7a6a', '#3d6d5d', '#4a8a6a', // Medium (front layer)
      ],
      fern: [
        '#3a7a5a', '#4a8a6a', '#5a9a7a', // Lighter fern greens
        '#4d8d6d', '#5d9d7d', '#6aaa8a',
      ],
    },
    // Depth layers for parallax jungle effect
    depthLayers: [
      { scale: 0.4, opacity: 0.3, speed: 0.3 },  // Far back - small, faded
      { scale: 0.6, opacity: 0.5, speed: 0.5 },  // Mid back
      { scale: 0.8, opacity: 0.7, speed: 0.7 },  // Mid front
      { scale: 1.0, opacity: 1.0, speed: 1.0 },  // Front - full size
    ]
  },
...
function PortraitEcosystem() {
  ...
  const entitiesRef = useRef({
    ...
    flowers: [],
    leaves: [],
    tropicalLeaves: [], // Large monstera/palm leaves
    ferns: [], // Delicate fern fronds
    wires: [],
    nodes: [],
    pulses: [],
    initialized: false,
