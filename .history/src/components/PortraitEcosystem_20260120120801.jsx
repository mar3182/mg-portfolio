...
const CONFIG = {
  ...
  circuit: {
    ...
    colors: {
      wire: ['#3a6a9a', '#4a7aaa', '#5a8aba'],
      node: ['#5a9ce4', '#6aacf4', '#7abcff'],
      pulse: ['#8accff', '#9adcff', '#aaf0ff'],
    }
  }
}
...
function PortraitEcosystem() {
  ...
  const animate = useCallback((time) => {
    ...
    // identity 0 = DESIGN (left) = show FLORA
    // identity 1 = TECH (right) = show CIRCUIT
    const currentIdentity = identity.get()
    // FIX: Swap the logic - flora visible when identity LOW (design), circuit when HIGH (tech)
    const floraOpacity = Math.max(0.15, 1 - currentIdentity)
    const circuitOpacity = Math.max(0.15, currentIdentity)
    
    // Get current mouse position
    const mouse = mouseRef.current
    
    // DEBUG: Draw a test circle to confirm canvas is working
    ctx.fillStyle = '#ff0000'
    ctx.beginPath()
    ctx.arc(100, 100, 20, 0, Math.PI * 2)
    ctx.fill()
    
    // Update and draw flora (with mouse wind effect)
    entities.vines.forEach(vine => {
    ...
    // Update and draw flowers (with mouse attraction)
    ...
    // Update and draw nodes (with mouse attraction)
