...
export default function ShowcaseGallery3D({ onExpand, onCollapse }) {
  ...
  const navigateStack = useCallback((direction) => {
    ...
    // Shorter throttle for responsive navigation
    wheelTimeoutRef.current = setTimeout(() => setIsNavigating(false), 150)
  }, [count, isNavigating])
  ...
  // Wheel navigation (throttled separately)
  const lastWheelTime = useRef(0)
  const handleWheel = useCallback((e) => {
    // Only handle wheel on the container
    if (!containerRef.current?.contains(e.target)) return
    
    // Throttle wheel events - 250ms between scrolls
    const now = Date.now()
