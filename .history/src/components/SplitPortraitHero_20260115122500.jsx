...
export default function SplitPortraitHero({ 
  ...
  // At center (0.5): rotateY = 90deg (edge-on, just a line)
  // At left (0): rotateY = 0deg (design face visible)
  // At right (1): rotateY = 180deg (tech face visible, but we flip the image)
  
  // The "unfold" rotation - from 90deg (edge) to 0deg or 180deg
  const rotateY = useTransform(smoothX, [0, 0.5, 1], [0, 90, 180])
  
  // Opacity of the portraits based on rotation
  // Design shows when rotated towards left (0-90deg)
  // Tech shows when rotated towards right (90-180deg)
  const designOpacity = useTransform(smoothX, [0, 0.4, 0.5], [1, 0.3, 0])
  const techOpacity = useTransform(smoothX, [0.5, 0.6, 1], [0, 0.3, 1])
  ...
  return (
    ...
    >
      ...
      <div className="t-shape-container">
