...
// ═══════════════════════════════════════════════════════════════════════════
// INDIVIDUAL ECHO COMPONENT (to use hooks properly)
// ═══════════════════════════════════════════════════════════════════════════

function EchoItem({ mouseX, mouseY, identity, config }) {
  const { image, depth, blur, opacity: baseOpacity, scale, offsetX, offsetY } = config
  
  // Parallax transforms
  const x = useTransform(mouseX, [0, 1], [offsetX - 30 * depth, offsetX + 30 * depth])
  const y = useTransform(mouseY, [0, 1], [offsetY - 20 * depth, offsetY + 20 * depth])
  
  // Fade based on identity
  const isDesignEcho = image.includes('design')
  const isTechEcho = image.includes('tech')
  
  const opacity = useTransform(identity, [0, 0.5, 1], [
    isDesignEcho ? baseOpacity * 1.5 : isTechEcho ? baseOpacity * 0.5 : baseOpacity,
    baseOpacity,
    isTechEcho ? baseOpacity * 1.5 : isDesignEcho ? baseOpacity * 0.5 : baseOpacity,
  ])
...
function EchoLayers({ mouseX, mouseY, identity }) {
  return (
    <div className="ecosystem-echoes" aria-hidden="true">
      {ECHO_CONFIGS.map((config) => (
        <EchoItem
          key={config.id}
          mouseX={mouseX}
          mouseY={mouseY}
...
// ═══════════════════════════════════════════════════════════════════════════
// INDIVIDUAL FRAGMENT COMPONENT (to use hooks properly)
// ═══════════════════════════════════════════════════════════════════════════

function FragmentItem({ mouseX, mouseY, identity, config }) {
  const { type, position, size, clipPath, depth, rotation, blur, opacity: baseOpacity } = config
  
  // Parallax based on depth
  const x = useTransform(mouseX, [0, 1], [-40 * depth, 40 * depth])
  const y = useTransform(mouseY, [0, 1], [-30 * depth, 30 * depth])
  
  // Subtle rotation response to mouse
  const rotate = useTransform(mouseX, [0, 1], [rotation - 3, rotation + 3])
  
  // Identity-based image crossfade
  const designOpacity = useTransform(identity, [0, 0.5, 1], [1, 0.5, 0])
  const techOpacity = useTransform(identity, [0, 0.5, 1], [0, 0.5, 1])
  
  return (
    <Motion.div
      className={`ecosystem-fragment ecosystem-fragment--${type}`}
      style={{
        ...position,
        ...size,
        x,
