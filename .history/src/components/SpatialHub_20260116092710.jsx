...
export default function SpatialHub({ 
  ...
  currentSection = 'center', // 'center' | 'design' | 'tech' | 'about'
  children, // The portrait content in the center
}) {
  const containerRef = useRef(null)
  const [activeHint, setActiveHint] = useState(null) // 'left' | 'right' | 'top' | 'bottom'
  ...
  const handleTouchEnd = useCallback((e) => {
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > threshold) {
      if (deltaX < 0) {
        onNavigate?.('tech') // Swipe left → go right (tech)
      } else {
        onNavigate?.('design') // Swipe right → go left (design)
      }
      ...
      if (deltaY < 0) {
        ...
        onNavigate?.('about') // Swipe down → about
      }
    }
  }, [onNavigate])
  ...
  return (
    ...
    >
      ...
      <Motion.div 
        ...
        style={{ opacity: leftEdgeProximity }}
      ...
      >
        <div className="edge-content">
          ...
          <div className="edge-preview">
            {SECTIONS.design.items.map((item, i) => (
              <span key={item} className="preview-item" style={{ animationDelay: `${i * 0.1}s` }}>
                {item}
              </span>
            ))}
          </div>
        </div>
        <div className="edge-glow" style={{ background: `linear-gradient(90deg, ${SECTIONS.design.color}20, transparent)` }} />
      </Motion.div>
      
      {/* === RIGHT EDGE: Tech Section === */}
      <Motion.div 
        className="spatial-edge spatial-edge--right"
        style={{ opacity: rightEdgeProximity }}
      ...
      >
        <div className="edge-content">
          ...
          <span className="edge-arrow">→</span>
          <div className="edge-preview">
            {SECTIONS.tech.items.map((item, i) => (
              <span key={item} className="preview-item" style={{ animationDelay: `${i * 0.1}s` }}>
                {item}
              </span>
            ))}
        ...
        <div className="edge-glow" style={{ background: `linear-gradient(-90deg, ${SECTIONS.tech.color}20, transparent)` }} />
      </Motion.div>
      
      {/* === TOP EDGE: About/Contact === */}
      <Motion.div 
        className="spatial-edge spatial-edge--top"
        style={{ opacity: topEdgeProximity }}
