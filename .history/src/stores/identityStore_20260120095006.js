...
const IDLE_TIMEOUT = null // Disabled - identity persists until user leaves hero
const IDLE_DRIFT_DURATION = 1.8 // Slow drift back to center (when enabled)
...
export const useIdentityStore = create(
  subscribeWithSelector((set, get) => ({
     ...
     * Currently DISABLED - identity persists until mouseLeave
     */
    resetIdleTimer: () => {
      ...
      // DISABLED: Idle reset was causing portrait switching when mouse stopped
      // The spec says "After ~2–3 seconds" but this feels disruptive
      // Identity now only resets on mouseLeave (handled in SplitPortraitHero)
