...
export default function TPortfolioHero({
  portraitCenter = `${import.meta.env.BASE_URL}portrait-split.png`,
  portraitDesign = `${import.meta.env.BASE_URL}portrait-design-left.png`,
  portraitTech = `${import.meta.env.BASE_URL}portrait-tech-right.png`,
  onNavigate,
  onIdentityCommit,
  isReady = true,
}) {
  ...
  const [cursorSide, setCursorSide] = useState('neutral') // 'design' | 'neutral' | 'tech'
  ...
  // Cursor ring springs (lagging ring effect)
  ...
  // ─── Vertical Unlock Logic + Cursor Side Tracking ───
  ...
  useEffect(() => {
    const unsubscribe = identity.on('change', (v) => {
      ...
      // Track which side the cursor is on for visual feedback
      if (v < CONFIG.zones.designCommit) setCursorSide('design')
      else if (v > CONFIG.zones.techCommit) setCursorSide('tech')
      else setCursorSide('neutral')
      // Emit the committed identity to the parent so sections adapt
      if (committed) {
        onIdentityCommit?.(v)
      }
    })
    return unsubscribe
  }, [identity, onIdentityCommit])
  ...
  const handleMouseLeave = useCallback(() => {
    // Gently return toward center so user doesn't get trapped on a side
    animateToIdentity(0.5, { duration: 1, source: 'hero-exit' })
    setShowCursor(false)
  }, [animateToIdentity])
  ...
  // ─── Return to Center ───
  const handleReturnToCenter = useCallback((e) => {
    e.stopPropagation()
    mouseX.set(0.5)
    animateToIdentity(0.5, { duration: 0.8, source: 'return-center' })
    onIdentityCommit?.(0.5)
  }, [mouseX, animateToIdentity, onIdentityCommit])
  ...
  return (
    ...
    >
      {/* ═══ CUSTOM CURSOR (always visible on dark bg) ═══ */}
      {showCursor && (
        <>
          <motion.div
            className={`t-hero__cursor t-hero__cursor--${cursorSide}`}
            style={{ x: cursorX, y: cursorY }}
          />
          <motion.div
            className={`t-hero__cursor-ring t-hero__cursor-ring--${cursorSide}`}
            style={{ x: smoothCursorX, y: smoothCursorY }}
          />
        </>
      )}
      {/* ═══ LAYER 0: IDENTITY BACKGROUND (Deep color base) ═══ */}
      <IdentityBackground
        warmOpacity={designWorldOpacity}
        coolOpacity={techWorldOpacity}
      />
      
      {/* ═══ LAYER 1: GRADIENT SHAPES (Large blurred blobs) ═══ */}
      <GradientShapes
      ...
      {/* ═══ LAYER 1.5: ATMOSPHERE (Textures overlay) ═══ */}
      <AtmosphereLayer
      ...
      {/* Nav sidebar & social links are now at page level (TPortfolioPage) */}
      ...
      {/* ═══ LAYER 8: RETURN TO CENTER BUTTON ═══ */}
      <AnimatePresence>
        {verticalUnlocked && (
          <motion.button
            className={`t-hero__return-center t-hero__return-center--${cursorSide}`}
          ...
          >
            <span className="return-center__icon">◎</span>
            <span className="return-center__label">Center</span>
          </motion.button>
        )}
      </AnimatePresence>
    </section>
...
/**
 * Identity Background — Deep warm/cool color base from living-portrait
 */
const IdentityBackground = memo(function IdentityBackground({ warmOpacity, coolOpacity }) {
  return (
    <div className="t-hero__identity-bg">
...
/**
 * Gradient Shapes — Large blurred color blobs from living-portrait
 */
const GradientShapes = memo(function GradientShapes({ designOpacity, techOpacity, parallaxX, parallaxY }) {
  const shapeX = useTransform(parallaxX, v => v * 1.4)
  const shapeY = useTransform(parallaxY, v => v * 1.4)
  const shapeXInv = useTransform(parallaxX, v => -v * 1.2)
  const shapeX2 = useTransform(parallaxX, v => v * 0.8)
  const shapeY2 = useTransform(parallaxY, v => v * 1.2)
  const shapeYSmall = useTransform(parallaxY, v => v * 0.9)
  
  return (
    <div className="t-hero__gradient-shapes">
      {/* Design shapes (warm — coral, orange) */}
      <motion.div className="gradient-shapes__group" style={{ opacity: designOpacity }}>
        <motion.div className="t-shape t-shape--coral" style={{ x: shapeX, y: shapeY }} />
        <motion.div className="t-shape t-shape--orange" style={{ x: shapeX2, y: shapeY2 }} />
      </motion.div>
      {/* Tech shapes (cool — purple, blue) */}
      <motion.div className="gradient-shapes__group" style={{ opacity: techOpacity }}>
        <motion.div className="t-shape t-shape--purple" style={{ x: shapeXInv, y: shapeY }} />
        <motion.div className="t-shape t-shape--blue" style={{ x: shapeXInv, y: shapeYSmall }} />
      </motion.div>
    </div>
  )
})
...
/**
 * Atmosphere Layer — Background textures overlay
