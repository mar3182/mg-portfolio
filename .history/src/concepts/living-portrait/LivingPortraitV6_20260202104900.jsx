...
export default function LivingPortraitV6() {
  ...
  // Vertical scroll position (which project within area)
  const scrollY = useMotionValue(0)
  const smoothY = useSpring(scrollY, { stiffness: 60, damping: 25 })
  ...
  const handleWheel = useCallback((e) => {
    ...
    const projectHeight = window.innerHeight * 0.8 // Each project takes 80vh
    
    // Determine current area
    const areaIndex = Math.round(currentX / sectionWidth) + CENTER_INDEX
    const currentArea = AREAS[areaIndex]
    const hasProjects = currentArea && currentArea.projects && currentArea.projects.length > 0
    
    // Calculate max vertical scroll for current area
    const maxY = hasProjects ? (currentArea.projects.length - 1) * projectHeight : 0
    
    // If we have projects and we're scrolling down, go vertical first
    if (hasProjects && delta > 0 && currentY < maxY) {
      // Scroll down through projects
      const newY = Math.min(currentY + delta * 0.8, maxY)
      scrollY.set(newY)
      return
    }
    
    // If we're scrolling up and have vertical offset, go back up first
    if (delta < 0 && currentY > 0) {
      const newY = Math.max(currentY + delta * 0.8, 0)
      scrollY.set(newY)
      return
    }
    ...
    const newX = Math.max(minX, Math.min(maxX, currentX + delta * 1.2))
    scrollX.set(newX)
    
    // Reset vertical when changing areas
    if (Math.abs(newX - currentX) > 10) {
      scrollY.set(0)
    }
  }, [scrollX, scrollY])

  // Track active area and project
  useEffect(() => {
    ...
    const unsubY = smoothY.on('change', (v) => {
      const projectHeight = window.innerHeight * 0.8
      const index = Math.round(v / projectHeight)
  ...
  }, [smoothX, smoothY, activeAreaIndex, activeProjectIndex])
  ...
  return (
    ...
    >
      ...
      {/* THE WORLD - moves horizontally AND vertically */}
      <motion.div 
      ...
      <ProjectIndicator 
        ...
        scrollY={smoothY}
      />
      <ScrollHint activeArea={activeArea} scrollY={smoothY} />
    </div>
  )
}
