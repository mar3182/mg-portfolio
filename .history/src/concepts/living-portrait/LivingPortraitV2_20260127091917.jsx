...
export default function LivingPortraitV2() {
  ...
  const designClipPath = useTransform(identity, (pos) => {
    if (pos >= 0.48) return 'inset(0 100% 0 0)' // Fully hidden
  ...
  const techClipPath = useTransform(identity, (pos) => {
    if (pos <= 0.52) return 'inset(0 0 0 100%)' // Fully hidden
    const revealPercent = ((pos - 0.52) / 0.48) * 100
    return `inset(0 0 0 ${100 - revealPercent}%)`
  })
  
  // Opacity to ensure portraits are truly hidden at center
  const designLayerOpacity = useTransform(identity, [0, 0.45, 0.5], [1, 1, 0])
  const techLayerOpacity = useTransform(identity, [0.5, 0.55, 1], [0, 1, 1])
