...
const CONFIG = {
  ...
  // World width in vw (how wide is the horizontal scroll world)
  worldWidth: 500,
  ...
  areaThreshold: 0.06,
  // Project spacing in vertical scroll (pixels between projects)
  projectSpacing: 180,
}
...
export default function LivingPortraitV5() {
  ...
  // Single unified scroll position (0 to 1)
  // This drives EVERYTHING - horizontal position AND depth
  ...
  // Handle scroll — Pure intuitive flow (Akaru-style)
  // Just scroll. The world responds naturally.
  const handleWheel = useCallback((e) => {
    e.preventDefault()
    
    const current = scrollProgress.get()
    const currentDepth = scrollY.get()
    
    // Combine all scroll input into one flow
    const scrollDelta = e.deltaY + e.deltaX * 0.5
    
    // When in an area, handle the T-navigation:
    // - Scrolling DOWN goes deeper into projects
    // - Scrolling UP from depth comes back to surface
    // - At surface (depth=0), scrolling UP/horizontal moves between areas
    if (activeArea) {
      const maxDepth = (activeArea.projects.length - 1) * CONFIG.projectSpacing
      
      if (scrollDelta > 0) {
        // Scrolling DOWN — go deeper into projects
        if (currentDepth < maxDepth) {
          const newDepth = Math.min(currentDepth + scrollDelta * 0.5, maxDepth)
          scrollY.set(newDepth)
          return
        }
        // At max depth, fall through to horizontal movement
      } else if (scrollDelta < 0) {
        // Scrolling UP
        if (currentDepth > 0) {
          // Come back up from depth
          const newDepth = Math.max(0, currentDepth + scrollDelta * 0.5)
          scrollY.set(newDepth)
          return
        }
        // At surface (depth=0), fall through to horizontal movement
      }
    }
    
    // Horizontal movement through the world
    const delta = scrollDelta * CONFIG.scrollSpeed
    const newProgress = Math.max(0, Math.min(1, current + delta))
    scrollProgress.set(newProgress)
    
    // Detect area naturally
  ...
  // Wider world = more horizontal travel (Akaru-style)
  // progress 0 = far left (Design), 0.5 = center, 1 = far right (Tech)
  const worldX = useTransform(
    smoothProgress, 
    [0, 0.5, 1], 
    [`${CONFIG.worldWidth / 2}vw`, '0vw', `-${CONFIG.worldWidth / 2}vw`]
  )
  ...
  return (
    ...
    >
      ...
      {/* THE SCROLLING WORLD — everything inside moves horizontally AND vertically */}
      <motion.div 
        className="lpv5-world" 
        style={{ 
          x: worldX,
          y: useTransform(verticalDepth, v => -v),
        }}
      >
        ...
        {/* Area content with integrated projects */}
        <AreaContent 
      ...
      <Navigation identity={smoothProgress} />
      
      {/* Minimal scroll hint - fades after first interaction */}
      <ScrollHint isLoaded={isLoaded} />
...
// ═══════════════════════════════════════════════════════════════════════════
// AREA CONTENT — Integrated markers + projects flowing in the world
// ═══════════════════════════════════════════════════════════════════════════

const AreaContent = memo(function AreaContent({ activeArea, verticalDepth, activeProject }) {
  ...
  return (
    <div className="lpv5-areas">
      {allAreas.map((area) => {
        ...
        // Use CONFIG.worldWidth for positioning
        const worldPosition = area.position * CONFIG.worldWidth
        
        return (
          <div
            key={area.id}
            className={`lpv5-area ${isActive ? 'lpv5-area--active' : ''} lpv5-area--${area.side}`}
            style={{
              left: `${worldPosition}vw`,
              '--area-color': area.color,
            }}
          >
            {/* Area header (visible at surface) */}
            <motion.div 
              className="lpv5-area__header"
              animate={{ 
                opacity: isActive ? 1 : 0.5,
                scale: isActive ? 1.05 : 1,
              }}
            >
              <span className="lpv5-area__icon">{area.icon}</span>
              <span className="lpv5-area__name">{area.name}</span>
              <motion.span 
                className="lpv5-area__line"
                animate={{ scaleY: isActive ? 1 : 0.3 }}
              />
            </motion.div>
            
            {/* Projects flow below (revealed by vertical scroll) */}
            <div className="lpv5-area__projects">
              {area.projects.map((project, index) => {
                const isProjectActive = isActive && activeProject?.id === project.id
                const projectY = (index + 1) * CONFIG.projectSpacing + 100 // Start below header
                
                return (
                  <motion.div
                    key={project.id}
                    className={`lpv5-project ${isProjectActive ? 'lpv5-project--active' : ''}`}
                    style={{
                      top: projectY,
                      '--project-index': index,
                    }}
                    ...
                    animate={{ 
                      ...
                      x: isProjectActive ? 0 : (area.side === 'design' ? -20 : 20),
                      scale: isProjectActive ? 1.02 : 1,
                    }}
                    transition={{ duration: 0.4, delay: index * 0.05 }}
                  >
                    <div className="lpv5-project__number">
                      {String(index + 1).padStart(2, '0')}
                    </div>
                    <div className="lpv5-project__content">
                      <h3 className="lpv5-project__name">{project.name}</h3>
                      <p className="lpv5-project__desc">{project.desc}</p>
                      <span className="lpv5-project__year">{project.year}</span>
                    </div>
                    <motion.div 
                      className="lpv5-project__bg"
                      animate={{ opacity: isProjectActive ? 1 : 0 }}
                    />
                  </motion.div>
                )
              })}
            ...
            {/* Scroll hint when active */}
            {isActive && (
              <motion.div 
                className="lpv5-area__scroll-hint"
                initial={{ opacity: 0 }}
                animate={{ opacity: 0.6 }}
...
// ═══════════════════════════════════════════════════════════════════════════
// SCROLL HINT — Minimal, disappears after interaction
// ═══════════════════════════════════════════════════════════════════════════

const ScrollHint = memo(function ScrollHint({ isLoaded }) {
  const [visible, setVisible] = useState(true)
  
  // Hide after 4 seconds or on first scroll
  useEffect(() => {
    const timer = setTimeout(() => setVisible(false), 4000)
    const hideOnScroll = () => setVisible(false)
    window.addEventListener('wheel', hideOnScroll, { once: true })
    return () => {
      clearTimeout(timer)
  ...
  return (
    <AnimatePresence>
      {isLoaded && visible && (
        ...
        >
          <motion.div 
            className="lpv5-scroll-hint__mouse"
            animate={{ y: [0, 8, 0] }}
            transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
          >
            <div className="lpv5-scroll-hint__wheel" />
          </motion.div>
          <span>Scroll</span>
        </motion.div>
      )}
