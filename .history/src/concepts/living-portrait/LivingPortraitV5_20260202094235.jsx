...
export default function LivingPortraitV5() {
  ...
  // Navigation mode: 'explore' (horizontal) or 'depth' (vertical within area)
  const [navMode, setNavMode] = useState('explore')
  ...
  // Handle scroll — Mode-aware navigation (Akaru-style)
  const handleWheel = useCallback((e) => {
    e.preventDefault()
    setShowModeHint(false) // Hide hint after first scroll
    
    const current = scrollProgress.get()
    
    // DEPTH MODE: When locked onto an area, vertical scroll explores projects
    if (navMode === 'depth' && activeArea) {
      const newScroll = scrollY.get() + e.deltaY * 0.8
      const maxScroll = (activeArea.projects.length - 1) * CONFIG.projectSpacing
      
      // If trying to scroll past the projects, switch back to explore mode
      if (newScroll < -50) {
        setNavMode('explore')
        scrollY.set(0)
        return
    ...
    // EXPLORE MODE: Horizontal navigation through the world
    ...
    // Detect if we've arrived at an area
    ...
    if (area !== activeArea) {
      ...
      // Auto-switch to depth mode when arriving at an area
      if (area) {
        // Small delay before enabling depth mode
        setTimeout(() => setNavMode('depth'), 300)
      }
    }
    
    // If leaving an area, ensure we're in explore mode
    if (!area && navMode === 'depth') {
      setNavMode('explore')
    }
  }, [scrollProgress, activeArea, scrollY, detectActiveArea, navMode])
