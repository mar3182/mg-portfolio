  ...
  return (
    ...
    >
      ...
      <main className="lpv4-main">
        {/* ═══ LAYER 6: DUALITY PORTRAIT ═══ */}
        <DualityPortrait identity={identity} />
        
        {/* ═══ LAYER 7: ANIMATED TYPOGRAPHY ═══ */}
        <AnimatedTitle identity={identity} isLoaded={isLoaded} />
        
        {/* ═══ LAYER 8: EXPLORATION ITEMS (visible at extremes) ═══ */}
...
const VideoBackgrounds = memo(function VideoBackgrounds({ identity }) {
  // Videos start invisible and slowly reveal as user explores extremes
  // Like a flashlight revealing hidden content in the dark
...
// ═══════════════════════════════════════════════════════════════════════════
// DUALITY PORTRAIT — Transforms based on exploration
// ═══════════════════════════════════════════════════════════════════════════

const DualityPortrait = memo(function DualityPortrait({ identity }) {
  // Portrait filters shift warm ↔ cool based on identity
  // Center (0.5): Balanced, slightly desaturated
  // Left (0): Warm, coral glow, artistic
  // Right (1): Cool, cyan tint, digital
  
  const hueRotate = useTransform(identity, [0, 0.5, 1], [-20, 0, 40])
  const saturate = useTransform(identity, [0, 0.5, 1], [1.3, 0.85, 0.7])
  const brightness = useTransform(identity, [0, 0.5, 1], [1.05, 0.95, 0.9])
  const grayscale = useTransform(identity, [0, 0.3, 0.5, 0.7, 1], [0, 0.1, 0.2, 0.1, 0])
  const contrast = useTransform(identity, [0, 0.5, 1], [0.95, 1, 1.1])
  
  // Warm glow (design side)
  const warmGlowOpacity = useTransform(identity, [0, 0.3, 0.5], [0.6, 0.3, 0])
  // Cool glow (tech side)
  const coolGlowOpacity = useTransform(identity, [0.5, 0.7, 1], [0, 0.3, 0.6])
  
  // Subtle scale based on engagement
  const scale = useTransform(identity, [0, 0.2, 0.5, 0.8, 1], [1.02, 1, 0.98, 1, 1.02])
  ...
  return (
    ...
    >
      ...
      <motion.div 
        className="lpv4-portrait__glow lpv4-portrait__glow--warm"
        style={{ opacity: warmGlowOpacity }}
      />
      
      {/* Cool glow behind portrait */}
      <motion.div 
        className="lpv4-portrait__glow lpv4-portrait__glow--cool"
        style={{ opacity: coolGlowOpacity }}
      />
      
      {/* Main portrait with dynamic filter */}
      <motion.div 
        className="lpv4-portrait__frame"
        style={{
          filter: useTransform(
            [hueRotate, saturate, brightness, grayscale, contrast],
            ([h, s, b, g, c]) => `hue-rotate(${h}deg) saturate(${s}) brightness(${b}) grayscale(${g}) contrast(${c})`
          )
        }}
      >
        <img 
      ...
      {/* Circuit overlay for tech side */}
      <motion.svg 
        className="lpv4-portrait__circuit"
        style={{ opacity: coolGlowOpacity }}
        viewBox="0 0 200 400"
        preserveAspectRatio="xMidYMid slice"
      >
        <path d="M60 0 C60 80, 40 120, 40 200 C40 280, 80 320, 60 400" stroke="rgba(0,229,255,0.4)" strokeWidth="1" fill="none" />
        <path d="M100 0 C100 100, 120 150, 100 250 C80 350, 120 380, 100 400" stroke="rgba(0,229,255,0.25)" strokeWidth="1" fill="none" />
        <path d="M140 0 C160 50, 140 100, 160 180 C180 260, 140 320, 160 400" stroke="rgba(0,229,255,0.3)" strokeWidth="1" fill="none" />
        <circle cx="60" cy="200" r="3" fill="rgba(0,229,255,0.6)" />
        <circle cx="100" cy="250" r="2.5" fill="rgba(0,229,255,0.5)" />
        <circle cx="160" cy="180" r="2" fill="rgba(0,229,255,0.4)" />
      </motion.svg>
    </motion.div>
  )
})
