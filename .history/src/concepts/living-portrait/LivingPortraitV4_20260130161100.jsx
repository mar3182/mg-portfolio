...
// ═══════════════════════════════════════════════════════════════════════════
// SPLIT PORTRAIT — Two portraits with clip-path reveal
// ═══════════════════════════════════════════════════════════════════════════

const SplitPortrait = memo(function SplitPortrait({ identity }) {
  // Clip path position (0% = full tech, 50% = split, 100% = full design)
  const clipPosition = useTransform(identity, [0, 0.5, 1], [100, 50, 0])
  
  // Design portrait opacity/visibility
  const designOpacity = useTransform(identity, [0, 0.3, 0.5], [1, 0.9, 0.7])
  const designScale = useTransform(identity, [0, 0.5, 1], [1.02, 1, 0.98])
  
  // Tech portrait opacity/visibility  
  const techOpacity = useTransform(identity, [0.5, 0.7, 1], [0.7, 0.9, 1])
  const techScale = useTransform(identity, [0, 0.5, 1], [0.98, 1, 1.02])
  
  // Glows
  const warmGlowOpacity = useTransform(identity, [0, 0.35, 0.5], [0.7, 0.4, 0])
  const coolGlowOpacity = useTransform(identity, [0.5, 0.65, 1], [0, 0.4, 0.7])
  
  return (
    <motion.div 
      className="lpv4-split-portrait"
    ...
    >
      ...
      <motion.div 
        className="lpv4-split-portrait__glow lpv4-split-portrait__glow--warm"
        style={{ opacity: warmGlowOpacity }}
      />
      
      {/* Cool glow behind portrait */}
      <motion.div 
        className="lpv4-split-portrait__glow lpv4-split-portrait__glow--cool"
        style={{ opacity: coolGlowOpacity }}
      />
      
      {/* Tech portrait (base layer) */}
      <motion.div 
        className="lpv4-split-portrait__layer lpv4-split-portrait__layer--tech"
        style={{ 
          opacity: techOpacity,
          scale: techScale,
        }}
      >
        <img 
          ...
          alt="Tech identity"
          className="lpv4-split-portrait__img"
        />
      </motion.div>
      
      {/* Design portrait (clips over tech) */}
      <motion.div 
        className="lpv4-split-portrait__layer lpv4-split-portrait__layer--design"
        style={{ 
