...
export default function LivingPortraitV5() {
  ...
  const handleWheel = useCallback((e) => {
    ...
    // If we're DEEP in an area (currentDepth > 0), handle vertical first
    if (activeArea && currentDepth > 0) {
      if (scrollDelta < 0) {
        // Scrolling up - come back up from depth
        const newDepth = Math.max(0, currentDepth + scrollDelta * 0.5)
        scrollY.set(newDepth)
        return
      } else {
        // Scrolling down - go deeper if possible
        ...
        // At max depth, let it fall through to horizontal
      }
    }
