...
function MatrixWorld({ identity, parallaxX, parallaxY, horizontalShift }) {
  const videoRef = useRef(null)
  
  // Opacity based on identity (visible on right/tech side)
  const opacity = useTransform(identity, [0.3, 0.5, 0.7, 1], [0, 0.3, 0.8, 1])
  
  // Horizontal scroll shift - use passed value or derive from identity
  const derivedShiftX = useTransform(identity, [0, 0.5, 1], ['-20vw', '0vw', '20vw'])
  ...
  // Parallax transforms - only if parallaxX/Y are provided
  const parallax1 = parallaxX ? {
    x: useTransform(parallaxX, v => -v * 0.5),
    y: useTransform(parallaxY, v => v * 0.5),
  } : { x: 0, y: 0 }
  const parallax2 = parallaxX ? {
  ...
  return (
    <motion.div 
      ...
      style={{ opacity, x: horizontalShift || derivedShiftX }}
    >
