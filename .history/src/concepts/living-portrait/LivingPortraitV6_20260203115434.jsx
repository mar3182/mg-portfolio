...
export default function LivingPortraitV6() {
  ...
  const verticalThreshold = 80 // pixels to scroll before triggering
    ...
    if (Math.abs(verticalScroll.current) >= verticalThreshold) {
      ...
      if (verticalScroll.current > 0) {
        ...
        if (currentProject < projectCount - 1) {
          // More projects in this slide → scroll down (VERTICAL)
          setDirection('down')
          setCurrentProject(prev => prev + 1)
          verticalScroll.current = 0
        } else if (currentSlide < SLIDES.length - 1) {
          // No more projects → move to next slide (HORIZONTAL)
          setDirection('right')
        ...
        // Scroll up
        if (currentProject > 0) {
          // Can scroll up within projects (VERTICAL)
          setDirection('up')
          setCurrentProject(prev => prev - 1)
          verticalScroll.current = 0
        } else if (currentSlide > 0) {
          // At top of projects → move to previous slide (HORIZONTAL)
          setDirection('left')
          setCurrentSlide(prev => prev - 1)
  ...
  return (
    <div ref={containerRef} className="akaru">
      ...
      {/* ══════════════════════════════════════════════════════════════════
          RIGHT PANEL — Horizontal Slides + Vertical Projects
          ══════════════════════════════════════════════════════════════════ */}
...
/* ═══════════════════════════════════════════════════════════════════════════
   SLIDE COMPONENT — Horizontal (areas) + Vertical (projects) animations
   ═══════════════════════════════════════════════════════════════════════════ */
...
const getAnimationVariants = (direction) => {
  ...
  if (isHorizontal) {
    // Horizontal animation for area changes
    return {
    ...
    // Vertical animation for project changes
    return {
      initial: { y: isForward ? '50%' : '-50%', opacity: 0 },
      animate: { y: 0, opacity: 1 },
      exit: { y: isForward ? '-50%' : '50%', opacity: 0 },
    }
  }
}

const Slide = memo(function Slide({ slide, project, direction, projectIndex, totalProjects }) {
    ...
    >
      ...
      <div className="akaru-slide__content">
        ...
        >
          ...
          <div className="akaru-slide__project-dots">
            {Array.from({ length: totalProjects }).map((_, i) => (
              <span 
                key={i} 
                className={`akaru-slide__project-dot ${i === projectIndex ? 'akaru-slide__project-dot--active' : ''}`}
                style={{ backgroundColor: i === projectIndex ? slide.color : 'rgba(255,255,255,0.3)' }}
              />
            ))}
          </div>
        </motion.div>
