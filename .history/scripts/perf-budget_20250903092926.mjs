...
// PERF_RATCHET=1 (apply changes), PERF_RATCHET=dry (preview only)
const ratchetMode = process.env.PERF_RATCHET
if (ratchetMode === '1' || ratchetMode === 'dry') {
  ...
  const isDryRun = ratchetMode === 'dry'
  let changes = []
  ...
  if (changes.length > 0) {
    console.log(`\n🔍 ${isDryRun ? 'DRY RUN - ' : ''}Budget changes${isDryRun ? ' that would apply' : ''}:`)
    for (const change of changes) {
      console.log(`  - ${change.label}: ${change.oldLimit} → ${change.newLimit} (current: ${change.currentSize})`)
      if (!isDryRun) {
        budgets[change.key] = change.newLimit
      }
    }
    ...
    console.log(`\n[perf-budget] ${isDryRun ? 'DRY RUN: ' : ''}No ratcheting changes (metrics not sufficiently below thresholds).`)
  }
}
