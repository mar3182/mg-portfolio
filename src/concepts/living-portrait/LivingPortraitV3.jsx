...
export default function LivingPortraitV3() {
  ...
  return (
    ...
    >
      ...
      {/* ═══ LAYER 5: ANIMATED TITLE — Splits apart as you explore ═══ */}
      <AnimatedTitle identity={identity} mouseX={mouseX} mouseY={mouseY} />
      
      {/* ═══ LAYER 6: ITEM DETAIL OVERLAY ═══ */}
      <AnimatePresence>
        {selectedItem && (
          <ItemDetail 
            item={selectedItem} 
...
// ═══════════════════════════════════════════════════════════════════════════
// ANIMATED TITLE — "Design + Tech" that splits as you explore
// ═══════════════════════════════════════════════════════════════════════════

const AnimatedTitle = memo(function AnimatedTitle({ identity, mouseX, mouseY }) {
  // Design word transforms - moves left and grows when exploring design zone
  const designX = useTransform(identity, [0, 0.5, 1], ['-35vw', '0vw', '25vw'])
  const designY = useTransform(identity, [0, 0.5, 1], ['-25vh', '0vh', '30vh'])
  const designScale = useTransform(identity, [0, 0.35, 0.5, 0.65, 1], [1.8, 1.3, 1, 0.6, 0.4])
  const designOpacity = useTransform(identity, [0, 0.3, 0.5, 0.7, 1], [1, 0.9, 0.7, 0.4, 0.2])
  const designRotate = useTransform(identity, [0, 0.5, 1], [-8, 0, 5])
  
  // Tech word transforms - moves right and grows when exploring tech zone
  const techX = useTransform(identity, [0, 0.5, 1], ['-25vw', '0vw', '35vw'])
  const techY = useTransform(identity, [0, 0.5, 1], ['30vh', '0vh', '-25vh'])
  const techScale = useTransform(identity, [0, 0.35, 0.5, 0.65, 1], [0.4, 0.6, 1, 1.3, 1.8])
  const techOpacity = useTransform(identity, [0, 0.3, 0.5, 0.7, 1], [0.2, 0.4, 0.7, 0.9, 1])
  const techRotate = useTransform(identity, [0, 0.5, 1], [5, 0, -8])
  
  // The "+" - starts in title, becomes the loupe
  // It follows the mouse more directly when away from center
  const plusScale = useTransform(identity, [0, 0.3, 0.5, 0.7, 1], [0.6, 0.8, 1.2, 0.8, 0.6])
  const plusOpacity = useTransform(identity, [0.35, 0.45, 0.55, 0.65], [0.3, 1, 1, 0.3])
  
  // Subtitle fades when exploring
  const subtitleOpacity = useTransform(identity, [0.3, 0.45, 0.55, 0.7], [0, 1, 1, 0])
  
  return (
    <div className="lpv3-animated-title">
      {/* Design word */}
      <motion.span 
        className="lpv3-animated-title__word lpv3-animated-title__word--design"
        style={{
          x: designX,
          y: designY,
          scale: designScale,
      ...
      {/* The "+" connector - stays more centered */}
      <motion.span 
        className="lpv3-animated-title__plus"
        style={{
          scale: plusScale,
          opacity: plusOpacity,
        }}
      ...
      {/* Tech word */}
      <motion.span 
        className="lpv3-animated-title__word lpv3-animated-title__word--tech"
        style={{
          x: techX,
          y: techY,
          scale: techScale,
      ...
      {/* Subtitle */}
      <motion.p 
        className="lpv3-animated-title__subtitle"
