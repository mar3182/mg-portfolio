...
export default function TPortfolioPage() {
  ...
  // Reset identity back to balanced (center)
  const handleResetIdentity = useCallback(() => {
    setCommittedSide('balanced')
...
const PageNav = memo(function PageNav({ activeSection, committedSide, onNavigate, onResetIdentity, isVisible }) {
  ...
  const isCommitted = committedSide !== 'balanced'
  ...
  return (
    ...
    >
      <button 
        className={`nav-sidebar__logo ${isCommitted ? 'nav-sidebar__logo--can-reset' : ''}`}
        onClick={onResetIdentity}
        title={isCommitted ? 'Return to balanced view' : 'Home'}
        aria-label={isCommitted ? 'Return to balanced view' : 'Home'}
      >
        <span>MG.</span>
        {isCommitted && <span className="nav-sidebar__reset-hint">◎</span>}
      </button>
