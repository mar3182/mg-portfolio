...
// ═══════════════════════════════════════════════════════════════════════════
// PORTRAIT STYLE CLASSES — Geometric & Illustrative Elements
// ═══════════════════════════════════════════════════════════════════════════

// GEOMETRIC CIRCLE — Sun/orb shapes like in the portrait
class GeometricCircle {
  constructor(x, y, config, depthLayer = null) {
    this.x = x
    this.y = y
    this.baseX = x
    this.baseY = y
    ...
    const baseSize = 20 + Math.random() * 60
    this.maxSize = baseSize * this.depth.scale
    this.size = 0
    this.growthProgress = 0
    
    this.color = config.colors.circle[Math.floor(Math.random() * config.colors.circle.length)]
    this.waveOffset = Math.random() * Math.PI * 2
    this.delay = Math.random() * 1000
    this.born = Date.now()
    
    // Some circles have rings, some are solid
    this.hasRing = Math.random() > 0.6
    this.ringWidth = 2 + Math.random() * 3
  }
  ...
  update(dt, waveTime) {
    ...
    if (this.growthProgress < 1) {
      this.growthProgress += dt * 0.0006 * this.depth.speed
    }
    // Smooth easeOutBack for organic pop-in
    const t = Math.min(1, this.growthProgress)
    const c1 = 1.70158
    const c3 = c1 + 1
    const eased = 1 + c3 * Math.pow(t - 1, 3) + c1 * Math.pow(t - 1, 2)
    this.size = this.maxSize * eased
    
    // Gentle float
    const float = Math.sin(waveTime * 0.3 + this.waveOffset) * 3 * this.depth.speed
  ...
  draw(ctx, opacity) {
    ...
    const layerOpacity = opacity * this.depth.opacity
    ctx.globalAlpha = layerOpacity * 0.7
    
    if (this.hasRing) {
      // Ring style (outline only)
      ctx.strokeStyle = this.color
      ctx.lineWidth = this.ringWidth * this.depth.scale
      ctx.beginPath()
      ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2)
      ctx.stroke()
    } else {
      // Solid circle with soft gradient
...
// SCATTERED DOT — Small accent dots
class ScatteredDot {
  constructor(x, y, config, depthLayer = null) {
    this.x = x
    this.y = y
    ...
    this.color = config.colors.dot[Math.floor(Math.random() * config.colors.dot.length)]
    this.waveOffset = Math.random() * Math.PI * 2
  ...
  update(dt, waveTime) {
    ...
    if (this.growthProgress < 1) {
      this.growthProgress += dt * 0.001 * this.depth.speed
    }
    this.size = this.maxSize * Math.min(1, this.growthProgress)
    
    // Subtle drift
    const drift = Math.sin(waveTime * 0.5 + this.waveOffset) * 2 * this.depth.speed
...
// PAINT SPLATTER — Organic splatter shapes
class PaintSplatter {
  constructor(x, y, config, depthLayer = null) {
    this.x = x
    this.y = y
    ...
    this.color = config.colors.splatter[Math.floor(Math.random() * config.colors.splatter.length)]
    this.rotation = Math.random() * Math.PI * 2
    this.delay = Math.random() * 1200
    this.born = Date.now()
    
    // Generate random splatter shape
    this.blobCount = 4 + Math.floor(Math.random() * 5)
    this.blobs = []
    for (let i = 0; i < this.blobCount; i++) {
      this.blobs.push({
        angle: (i / this.blobCount) * Math.PI * 2 + (Math.random() - 0.5) * 0.5,
  ...
  update(dt) {
    ...
    if (this.growthProgress < 1) {
      this.growthProgress += dt * 0.0004 * this.depth.speed
    }
    this.size = this.maxSize * Math.min(1, this.growthProgress)
  }
  ...
  draw(ctx, opacity) {
    ...
    const layerOpacity = opacity * this.depth.opacity * 0.5
    ctx.globalAlpha = layerOpacity
    ctx.fillStyle = this.color
    ...
    // Scattered blobs around center
    this.blobs.forEach(blob => {
      const bx = Math.cos(blob.angle) * this.size * blob.distance
      const by = Math.sin(blob.angle) * this.size * blob.distance
      const bs = this.size * blob.size
...
// STYLIZED BRANCH — Branch with simple oval leaves (like in portrait)
class StylizedBranch {
  constructor(x, y, angle, config, depthLayer = null) {
    this.x = x
    this.y = y
    this.baseX = x
    ...
    const baseLength = 60 + Math.random() * 100
    this.maxLength = baseLength * this.depth.scale
    this.length = 0
    this.growthProgress = 0
    
    this.branchColor = config.colors.branch[Math.floor(Math.random() * config.colors.branch.length)]
    // Mix of navy and orange leaves like in portrait
    this.leafColors = Math.random() > 0.4 
      ? config.colors.leaf.slice(0, 3) // Navy leaves
      : config.colors.leaf.slice(3) // Orange leaves
    
    this.waveOffset = Math.random() * Math.PI * 2
    this.delay = Math.random() * 600
    this.born = Date.now()
    
    // Generate leaf positions along branch
    this.leafCount = 4 + Math.floor(Math.random() * 5)
    this.leaves = []
    for (let i = 0; i < this.leafCount; i++) {
      this.leaves.push({
        t: 0.2 + (i / this.leafCount) * 0.7, // Position along branch
  ...
  update(dt, waveTime) {
    ...
    this.length = this.maxLength * Math.min(1, this.growthProgress)
    
    // Gentle sway
    const sway = Math.sin(waveTime * 0.4 + this.waveOffset) * 0.05 * this.depth.speed
    this.currentAngle = this.angle + sway
  }
  ...
  draw(ctx, opacity) {
    ...
    ctx.save()
    ctx.translate(this.x, this.y)
    ctx.rotate(this.currentAngle || this.angle)
    
    // Draw curved branch stem
    ctx.strokeStyle = this.branchColor
    ctx.lineWidth = 2 * this.depth.scale
    ctx.lineCap = 'round'
    ctx.globalAlpha = layerOpacity * 0.8
    ...
    ctx.moveTo(0, 0)
    ctx.quadraticCurveTo(
      this.length * 0.5, -this.length * 0.1,
      this.length, 0
    )
    ctx.stroke()
    
    // Draw simple oval leaves
    ctx.globalAlpha = layerOpacity * 0.85
    this.leaves.forEach(leaf => {
      if (leaf.t > this.growthProgress) return // Don't draw leaves beyond growth
      
      const leafX = this.length * leaf.t
      const leafY = -this.length * 0.1 * Math.sin(leaf.t * Math.PI) // Follow curve
      const leafSize = 12 * this.depth.scale * leaf.size
      ...
      ctx.ellipse(leafSize * 0.6, 0, leafSize, leafSize * 0.35, 0, 0, Math.PI * 2)
      ctx.fill()
      
      // Optional: thin line through leaf
      ctx.strokeStyle = 'rgba(255,255,255,0.15)'
      ctx.lineWidth = 0.5
      ctx.beginPath()
      ctx.moveTo(0, 0)
      ctx.lineTo(leafSize * 1.1, 0)
      ctx.stroke()
...
function PortraitEcosystem() {
  ...
  const initializeEntities = useCallback((canvas) => {
    ...
    CONFIG.floraOrigins.forEach(origin => {
      ...
      for (let i = 0; i < vinesPerOrigin; i++) {
        ...
        const angle = baseAngle + (Math.random() - 0.5) * spread - (origin.x * 0.5)
        const offsetX = (Math.random() - 0.5) * 80
        const offsetY = (Math.random() - 0.5) * 80
        entities.vines.push(new Vine(canvas, originX + offsetX, originY + offsetY, angle, CONFIG.flora))
      }
      
      // GEOMETRIC CIRCLES — Orange/golden sun orbs
      const circlesPerOrigin = Math.ceil(CONFIG.flora.maxCircles / CONFIG.floraOrigins.length)
      for (let i = 0; i < circlesPerOrigin; i++) {
        const offsetX = (Math.random() - 0.5) * 300
        const offsetY = (Math.random() - 0.5) * 300
        const depthLayer = CONFIG.flora.depthLayers[Math.floor(Math.random() * CONFIG.flora.depthLayers.length)]
        entities.circles.push(new GeometricCircle(originX + offsetX, originY + offsetY, CONFIG.flora, depthLayer))
      }
      
      // SCATTERED DOTS — Small accent dots
      const dotsPerOrigin = Math.ceil(CONFIG.flora.maxDots / CONFIG.floraOrigins.length)
      for (let i = 0; i < dotsPerOrigin; i++) {
        const offsetX = (Math.random() - 0.5) * 350
        const offsetY = (Math.random() - 0.5) * 350
        const depthLayer = CONFIG.flora.depthLayers[Math.floor(Math.random() * CONFIG.flora.depthLayers.length)]
        entities.dots.push(new ScatteredDot(originX + offsetX, originY + offsetY, CONFIG.flora, depthLayer))
      }
      
      // PAINT SPLATTERS — Organic splatter shapes
      const splattersPerOrigin = Math.ceil(CONFIG.flora.maxSplatters / CONFIG.floraOrigins.length)
      for (let i = 0; i < splattersPerOrigin; i++) {
        const offsetX = (Math.random() - 0.5) * 250
        const offsetY = (Math.random() - 0.5) * 250
        const depthLayer = CONFIG.flora.depthLayers[Math.floor(Math.random() * CONFIG.flora.depthLayers.length)]
        entities.splatters.push(new PaintSplatter(originX + offsetX, originY + offsetY, CONFIG.flora, depthLayer))
      }
      
      // STYLIZED BRANCHES — Branches with simple oval leaves
      const branchesPerOrigin = Math.ceil(CONFIG.flora.maxBranches / CONFIG.floraOrigins.length)
      for (let i = 0; i < branchesPerOrigin; i++) {
        const angle = -Math.PI / 2 + (Math.random() - 0.5) * Math.PI * 0.8 // Mostly upward
        const offsetX = (Math.random() - 0.5) * 200
        const offsetY = (Math.random() - 0.5) * 200
        const depthLayer = CONFIG.flora.depthLayers[Math.floor(Math.random() * CONFIG.flora.depthLayers.length)]
        entities.branches.push(new StylizedBranch(originX + offsetX, originY + offsetY, angle, CONFIG.flora, depthLayer))
      }
      
      // SIMPLE LEAVES — Scattered oval silhouettes
  ...
  const animate = useCallback((time) => {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // FLORA RENDERING — Portrait illustration style
    // Geometric shapes, simple leaves, paint splatters, flowing lines
    // ═══════════════════════════════════════════════════════════════════════
    
    // BACK LAYER: Paint splatters (atmospheric background)
    const sortedSplatters = [...entities.splatters].sort((a, b) => a.depth.scale - b.depth.scale)
    sortedSplatters.forEach(splatter => {
      splatter.update(dt)
      const boost = getMouseBoost(splatter.x, splatter.y)
      splatter.draw(ctx, (floraBaseOpacity + boost) * breathOpacity)
    })
    
    // MID-BACK LAYER: Geometric circles (sun orbs)
    const sortedCircles = [...entities.circles].sort((a, b) => a.depth.scale - b.depth.scale)
    sortedCircles.forEach(circle => {
      circle.update(dt, entities.waveTime)
      const boost = getMouseBoost(circle.x, circle.y)
      circle.draw(ctx, (floraBaseOpacity + boost) * breathOpacity)
    })
    
    // MID LAYER: Stylized branches with leaves
    const sortedBranches = [...entities.branches].sort((a, b) => a.depth.scale - b.depth.scale)
    sortedBranches.forEach(branch => {
      branch.update(dt, entities.waveTime)
      const boost = getMouseBoost(branch.x, branch.y)
      branch.draw(ctx, (floraBaseOpacity + boost) * breathOpacity)
    })
    
    // MID LAYER: Flowing vines/lines
    entities.vines.forEach(vine => {
    ...
    // MID-FRONT LAYER: Scattered leaves (simple ovals)
    ...
    // FRONT LAYER: Scattered dots (accent)
    const sortedDots = [...entities.dots].sort((a, b) => a.depth.scale - b.depth.scale)
    sortedDots.forEach(dot => {
      dot.update(dt, entities.waveTime)
      const boost = getMouseBoost(dot.x, dot.y)
      dot.draw(ctx, (floraBaseOpacity + boost) * breathOpacity)
    })
    
    // TOP LAYER: Flowers (bold color accents)
